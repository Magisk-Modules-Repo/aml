...
installmod() {
  ...
  if $BOOTMODE; then
    ...
    rm -f $MAGISKTMP/img/$MODID/remove; mktouch $MAGISKTMP/img/$MODID/update
  ...
  cp_mv -c $INSTALLER/common/aml.sh $COREPATH/service.d/aml.sh 0755
...
uninstallmod() {
  ...
  if $BOOTMODE; then
    ...
    local MODDIRS="$MOUNTPATH $MAGISKTMP/img"
  else
